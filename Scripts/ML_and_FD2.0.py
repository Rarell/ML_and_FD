...
def execute_sklearn_exp(args, train_in, valid_in, test_in, train_out, valid_out, test_out, model_fname, evaluate_each_grid = False):
    '''
    ...
    # Evaluate the model
    results = sklearn_evaluate_model(model, args, train_in, valid_in, test_in, train_out, valid_out, test_out, IJ, T, Tt)
    
    # Evaluate the model for each grid point?
    if evaluate_each_grid:
        eval_train_map = np.zeros((IJ, len(arg.metrics))) * np.nan
        eval_valid_map = np.zeros((IJ, len(arg.metrics))) * np.nan
        eval_test_map = np.zeros((IJ, len(arg.metrics))) * np.nan
        ...
        for ij in range(IJ):
            r_tmp = sklearn_evaluate_model(model, args, train_in[:,:,ij], valid_in[:,:,ij], test_in[:,:,ij], 
                                           train_out[:,ij], valid_out[:,ij], test_out[:,ij], IJ, T, Tt)
            
            eval_train_map[ij,:] = r_tmp['train_eval']
            eval_valid_map[ij,:] = r_tmp['valid_eval']
            eval_test_map[ij,:] = r_tmp['test_eval']
...
def execute_single_exp(args, train_in, valid_in, test_in, train_out, valid_out, test_out, model_fname, evaluate_each_grid = False):
    '''
    ...
    if args.keras:
        ...
        results = execute_sklearn_exp(args, train_in, valid_in, test_in, train_out, valid_out, test_out, model_fname, evaluate_each_grid)
    ...
    for method in range(Nmethods):
        ...
        for n, (region_in, region_out) in enumerate(zip(data_in_split, data_out_split)):
            ...
            for rot in rotation:
                ...
                model_fname = '%s/%s/%s/%s'%(dataset_dir, args.ml_model, methods[method], model_fbase)
            ...
            if n%10 == 0:
                print('Evaluating the %s for the %dth region with the %s method...'%(args.ml_model, n+1, methods[method]))
